...
def prepare_data(df):
    ...
    df['action'] = df['action'].apply(
        lambda x: 1.0 if x == 'purchased' else (0.3 if x == 'addCart' else 0.6)
    )

    return df, user_encoder, item_encoder

def recommend_items_with_scores(model, user_id, user_encoder, item_encoder, top_k=8):
    model.eval()
...
def recommend_popular_items(item_encoder, model, top_k=8):
    all_items = np.arange(len(item_encoder.classes_))
...
def calculate_content_similarity(target_product_id, products_df, top_k=8):
...
def hybrid_recommendations(user_id, model, user_encoder, item_encoder, products_df, alpha=0.7, top_k=8):
    ...
    for product_id, cf_score in cf_results:
        cbf_results = calculate_content_similarity(product_id, products_df, top_k=8)
...
def train_ncf(df, test_size=0.2, epochs=50, lr=0.001, model_path='ncf_model.pth'):
    ...
    train_df, test_df = train_test_split(df, test_size=test_size) if len(df) > 1 else (df, pd.DataFrame())
...
def get_recommendations(user_id, products_df):
    ...
    try:
        if not os.path.exists(user_log_path):
            print(f"Error: File {user_log_path} does not exist.")
            return
    ...
    if df.empty:
        print(f"No interaction data available for user {user_id}.")
        return
    ...
    try:
        ...
        print(f"Error during hybrid recommendation generation: {e}")
        return

    recommendations_data = [{"product_id": int(product_id)} for product_id in recommendations][-8:][::-1]
